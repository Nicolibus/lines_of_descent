...
function intro_03_bottleneck(){
    ...
    if(checkBottleneck()){
        ...
        const bottleneck = d3.select("#bottleneck")
            .selectAll("polygon")
            .nodes()
            .find((node,n,l) =>node.getAttribute("fill") === "rgba(0,0,0,0)" && +node.getAttribute("data-position") !== 0 && +node.getAttribute("data-position") !== +node.getAttribute("data-rowMax") && (l[n-1]? (l[n-1].getAttribute("fill") !== "rgba(0,0,0,0)") : false) && (l[n+1]? (l[n+1].getAttribute("fill") !== "rgba(0,0,0,0)") : false)  )
            //.find(node => console.log(node.getAttribute("data-position") , +node.getAttribute("data-rowMax"))  )
            ?.getBoundingClientRect()
        ...
        .append("line")
            ...
            .transition()
            .attr("y1", 188)
            .attr("x1",(isMobile?window.innerWidth / 8:window.innerWidth / 3)+25)
            .attr("stroke","var(--gray)")
